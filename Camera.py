...
class Camera(object):
    ...
    def empty(self):
        return type(self.image_raw).isinstance(type(None))
    ...
    def enable_write(self, output_path, image):
        ...
        self.video_writer = cv2.VideoWriter((self.base_location+output_path),
                                            fourcc, self.frame_rate,
                                            (width, height))

    def enable_detection_write(self, output_path, image):
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        height, width = image.shape[:2]
        self.detection_video_writer = cv2.VideoWriter(
            (self.base_location+output_path),
            fourcc,
            self.frame_rate,
            (width, height))
